"""Map lyrics text to audio timestamps using WhisperX forced alignment."""

import json
from pathlib import Path


def _parse_lyrics(lyrics_path: str) -> list[str]:
    """Read lyrics file, return list of non-empty lines."""
    text = Path(lyrics_path).read_text(encoding="utf-8")
    return [line.strip() for line in text.splitlines() if line.strip()]


def _parse_lyrics_with_header(lyrics_path: str) -> tuple[list[str], list[str]]:
    """Parse lyrics file, separating header (title + markers) from body."""
    text = Path(lyrics_path).read_text(encoding="utf-8")
    lines = [line.strip() for line in text.splitlines()]
    non_empty = [l for l in lines if l]

    if not non_empty:
        return ([], [])

    header_end = 0
    for i, line in enumerate(non_empty):
        if line.startswith("[") and line.endswith("]"):
            header_end = i + 1
        elif header_end > 0:
            break

    if header_end > 0:
        return (non_empty[:header_end], non_empty[header_end:])

    return ([], non_empty)


def _is_marker_line(text: str) -> bool:
    """Check if a lyrics line is a bracket marker like [前奏]."""
    stripped = text.strip()
    return stripped.startswith("[") and stripped.endswith("]")


def _interpolate_chars(text: str, start: float, end: float) -> list[dict]:
    """Evenly distribute char timings across the given span."""
    if not text:
        return []
    n = len(text)
    duration = (end - start) / n
    return [
        {"char": c, "start": round(start + i * duration, 3), "end": round(start + (i + 1) * duration, 3)}
        for i, c in enumerate(text)
    ]


def align_force(aligned_lines: list[dict], header_lines: list[str], first_audio_time: float) -> dict:
    """Build final aligned output from force_align_lyrics results + header lines."""
    result_lines = []

    # Header lines get interpolated timing before first audio
    for i, line_text in enumerate(header_lines):
        if first_audio_time > 0:
            start = round(max(first_audio_time - 5.0, 0.0), 3)
            end = round(first_audio_time - 0.1, 3)
        else:
            start, end = 0.0, 0.0
        result_lines.append({
            "line_no": i + 1,
            "text": line_text,
            "start": start,
            "end": end,
            "chars": _interpolate_chars(line_text, start, end),
        })

    # Body lines from force alignment
    line_no_offset = len(header_lines)
    for i, aligned in enumerate(aligned_lines):
        warn = {"warning": aligned["warning"]} if "warning" in aligned else {}
        result_lines.append({
            "line_no": line_no_offset + i + 1,
            "text": aligned["text"],
            "start": aligned["start"],
            "end": aligned["end"],
            "chars": aligned["chars"],
            **warn,
        })

    return {"lines": result_lines}


def align_lyrics(transcript: dict, lyrics_path: str) -> dict:
    """Fallback: align using SequenceMatcher (used when force alignment not available).

    This is the legacy path. Prefer force_align_lyrics in transcribe.py instead.
    """
    header_lines, body_lines = _parse_lyrics_with_header(lyrics_path)
    char_seq = _flatten_chars(transcript)

    result_lines = []
    search_start = 0

    # Header markers
    for i, line_text in enumerate(header_lines):
        result_lines.append({
            "line_no": i + 1, "text": line_text,
            "start": 0.0, "end": 0.0, "chars": [], "marker": True,
        })

    # Body lines
    line_no_offset = len(header_lines)
    for i, line_text in enumerate(body_lines):
        line_no = line_no_offset + i + 1

        if _is_marker_line(line_text):
            result_lines.append({
                "line_no": line_no, "text": line_text,
                "start": 0.0, "end": 0.0, "chars": [], "marker": True,
            })
            continue

        span = _find_match_span(line_text, char_seq, search_start)
        if span:
            start_idx, end_idx = span
            matched_chars = char_seq[start_idx:end_idx]
            line_chars = _build_line_chars(line_text, matched_chars)
            actual_start = line_chars[0]["start"] if line_chars else matched_chars[0]["start"]
            actual_end = line_chars[-1]["end"] if line_chars else matched_chars[-1]["end"]
            result_lines.append({
                "line_no": line_no, "text": line_text,
                "start": actual_start, "end": actual_end, "chars": line_chars,
            })
            search_start = end_idx
        else:
            start_time = 0.0
            for prev in reversed(result_lines):
                if "marker" not in prev and prev.get("end", 0) > 0:
                    start_time = prev["end"] + 0.1
                    break
            end_time = start_time + max(len(line_text) * 0.3, 2.0) if start_time > 0 else 0
            result_lines.append({
                "line_no": line_no, "text": line_text,
                "start": round(start_time, 3), "end": round(end_time, 3),
                "chars": _interpolate_chars(line_text, start_time, end_time),
                "warning": "unmatched",
            })

    # Assign timing to markers
    first_match_start = None
    for line in result_lines:
        if "marker" not in line and line.get("start", 0) > 0:
            first_match_start = line["start"]
            break
    for line in result_lines:
        if "marker" in line:
            if first_match_start and first_match_start > 0:
                line["start"] = round(max(first_match_start - 5.0, 0.0), 3)
                line["end"] = round(first_match_start - 0.1, 3)
                line["chars"] = _interpolate_chars(line["text"], line["start"], line["end"])
            del line["marker"]

    return {"lines": result_lines}


def _flatten_chars(transcript: dict) -> list[dict]:
    chars = []
    for seg in transcript["segments"]:
        chars.extend(seg.get("chars", []))
    return chars


def _find_match_span(lyrics_line: str, char_seq: list[dict], search_start: int) -> tuple[int, int] | None:
    if not lyrics_line or not char_seq:
        return None
    seq_text = "".join(c["char"] for c in char_seq)
    remaining_text = seq_text[search_start:]
    if not remaining_text:
        return None
    min_match = max(len(lyrics_line) * 0.4, 2)
    from difflib import SequenceMatcher
    sm = SequenceMatcher(None, lyrics_line, remaining_text)
    for match in sorted(sm.get_matching_blocks(), key=lambda m: m.b):
        if match.size >= min_match:
            start_idx = search_start + match.b
            end_idx = start_idx + match.size
            return (start_idx, end_idx)
    return None


def _build_line_chars(line_text: str, matched_chars: list[dict]) -> list[dict]:
    if len(matched_chars) == len(line_text):
        return [{"char": c["char"], "start": c["start"], "end": c["end"]} for c in matched_chars]

    from difflib import SequenceMatcher
    matched_text = "".join(c["char"] for c in matched_chars)
    sm = SequenceMatcher(None, line_text, matched_text)

    line_chars = []
    for tag, i1, i2, j1, j2 in sm.get_opcodes():
        if tag == "equal":
            for k in range(i2 - i1):
                mc = matched_chars[j1 + k]
                line_chars.append({"char": line_text[i1 + k], "start": mc["start"], "end": mc["end"]})
        elif tag == "replace":
            for k in range(i2 - i1):
                mc = matched_chars[j1 + min(k, j2 - j1 - 1)]
                line_chars.append({"char": line_text[i1 + k], "start": mc["start"], "end": mc["end"]})
        elif tag == "insert":
            n = i2 - i1
            prev_end = line_chars[-1]["end"] if line_chars else matched_chars[0]["start"]
            next_start = matched_chars[j1]["start"] if j1 < len(matched_chars) else prev_end + n * 0.3
            if next_start <= prev_end:
                next_start = prev_end + n * 0.3
            d = (next_start - prev_end) / n
            for k in range(n):
                line_chars.append({"char": line_text[i1 + k], "start": round(prev_end + k * d, 3), "end": round(prev_end + (k + 1) * d, 3)})
        elif tag == "delete":
            n = i2 - i1
            if matched_chars:
                if j1 == 0 and not line_chars:
                    first_start = matched_chars[0]["start"]
                    d = min(0.3, first_start / max(n, 1))
                    for k in range(n):
                        line_chars.append({"char": line_text[i1 + k], "start": round(first_start - (n - k) * d, 3), "end": round(first_start - (n - k - 1) * d, 3)})
                elif j1 < len(matched_chars):
                    prev_end = line_chars[-1]["end"] if line_chars else matched_chars[max(j1 - 1, 0)]["end"]
                    next_start = matched_chars[j1]["start"]
                    if next_start <= prev_end:
                        next_start = prev_end + n * 0.3
                    d = (next_start - prev_end) / n
                    for k in range(n):
                        line_chars.append({"char": line_text[i1 + k], "start": round(prev_end + k * d, 3), "end": round(prev_end + (k + 1) * d, 3)})
                else:
                    prev_end = line_chars[-1]["end"] if line_chars else 0
                    d = 0.3
                    for k in range(n):
                        line_chars.append({"char": line_text[i1 + k], "start": round(prev_end + k * d, 3), "end": round(prev_end + (k + 1) * d, 3)})
    return line_chars


def main():
    """CLI: align <transcript.json> <lyrics.txt> [--output aligned.json]"""
    import argparse

    parser = argparse.ArgumentParser(description="Align lyrics to transcript")
    parser.add_argument("transcript", help="Path to transcript.json")
    parser.add_argument("lyrics", help="Path to lyrics.txt")
    parser.add_argument("--output", default="aligned.json")
    args = parser.parse_args()

    transcript = json.loads(Path(args.transcript).read_text(encoding="utf-8"))
    result = align_lyrics(transcript, args.lyrics)
    Path(args.output).write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Aligned lyrics saved to {args.output}")
